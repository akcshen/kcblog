...
一是**浏览器兼容降级**，通过 Babel 转译、core-js 垫片、环境判断，保证老旧设备不白屏；

二是**低性能设备与弱网降级**，通过懒加载、虚拟滚动、关闭非必要动画、接口缓存等方式，保证基础功能可用；

同时对异常场景做**容错降级**，使用错误边界和接口兜底，避免局部问题影响整个页面。

### 37.遇到的最大的挑战

解构导致响应式丢失，要使用 **toRefs**包一下
